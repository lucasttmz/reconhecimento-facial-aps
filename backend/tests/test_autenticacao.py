...
def test_login_sem_imagens_suficientes(cliente):
    resposta = cliente.post("http://127.0.0.1:8000/login", json={"fotos": []})
...
def test_login_imagens_formato_invalido(cliente):
    resposta = cliente.post("http://127.0.0.1:8000/login", json={"fotos": ["a"] * 10})
    assert resposta.status_code == HTTPStatus.UNPROCESSABLE_ENTITY
...
def test_tentativa_de_login_invalida(cliente, imagem_base64):
    resposta = cliente.post("http://127.0.0.1:8000/login", json={"fotos": [imagem_base64]})
...
def test_registro_sem_rostos(cliente, imagem_base64):
    ...
    assert resposta.status_code == HTTPStatus.UNPROCESSABLE_ENTITY
    assert resposta.json().get("detail") == "Centralizar o rosto antes de tirar as fotos"
